...
async function gettime() {
  let Time = await fetch('http://worldtimeapi.org/api/timezone/Indian/Cocos');
  let Response = await Time.json();
  if (Response.datetime.slice(11, 13) >= 7 && Response.datetime.slice(11, 13) <=11) {
      backgroundC =rgb(238,242,7);
  } else {
      backgroundC = rgb(0,0,0);
  }
}